...
public enum Tier: String, Sendable, Comparable, CaseIterable {
    ...
    case basic = "BASIC"
    /// Adds match events and market prices.
    case pro = "PRO"
    /// Adds model analysis, live model fields and the WebSocket feed.
    case ultra = "ULTRA"
...
/// The lowest tier that unlocks an endpoint, inferred from the path.
///
...
/// endpoint it called. First match wins. Kept identical to the Python, JS, Go
/// and .NET clients.
func requiredTier(forPath path: String) -> Tier? {
...
public enum LiveTennisApiError: Error, CustomStringConvertible {
    ...
    public var info: ApiErrorInfo? {
        ...
        case .badRequest(let info, _), .unauthorized(let info),
            ...
            .rateLimited(let info, _), .serverError(let info), .status(let info):
            return info
    ...
    public var description: String {
        ...
        case .unauthorized(let info), .notFound(let info),
            .serverError(let info), .status(let info):
            return line(info)
        ...
        case .rateLimited(let info, let retryAfter):
            if let retryAfter {
                return "\(line(info)) — retry after \(retryAfter)s"
            }
            return line(info)
    ...
    static func forStatus(
        _ status: Int, path: String, url: String, rateLimit: RateLimit, body: Data
    ) -> LiveTennisApiError {
        ...
        case 403: return .upgradeRequired(info, requiredTier: requiredTier(forPath: path))
        case 404: return .notFound(info)
        case 429: return .rateLimited(info, retryAfter: rateLimit.retryAfter)
